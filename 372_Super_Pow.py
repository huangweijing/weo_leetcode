...
class Solution:
    # @cache
    def my_super_pow(self, a:int, b: List[int]):
        if len(b) == 0:
            return 1
        dig = b.pop()
        result = 1
        result = result * (a ** dig % 1337)

        sub = self.my_super_pow(a, b)
        result = result * (sub ** 10 % 1337)
