...
class WeoRegular:
    def __init__(self, re: str):
        ...
        for idx in range(len(re) - 1):
            ...
            # if "a" <= re[idx] <= "z" or re[idx] == "." and re[idx + 1] != "*":
            #     self.g.add_edge(idx, idx + 1)
            if idx + 1 < len(re) and re[idx + 1] == "*":
                self.g.add_edge(idx + 1, idx)
            if idx + 2 < len(re) and re[idx + 2] == "*":
    ...
    def dg_nfs(self, step_list: list[str]) -> set[int]:
        ...
        for step in step_list:
            ...
            for state in available_state:
                if self.re[state] == step or self.re[state] == ".":
...
weo_re = WeoRegular("a.*c")
r = weo_re.is_match("ac")
print(r)
