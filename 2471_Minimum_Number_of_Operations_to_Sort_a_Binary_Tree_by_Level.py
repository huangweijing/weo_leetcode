...
class Solution:
    def count_operation(self, arr: list[int]) -> int:
        ...
        arr_sorted = arr.copy()
        arr_sorted.sort()
        dic = { key: val for val, key in enumerate(arr) }
        # print(arr)
        ans = 0
        for idx in range(len(arr)):
            if arr[idx] != arr_sorted[idx]:
                exchange_idx = dic[arr_sorted[idx]]
                dic[arr[idx]], dic[arr[exchange_idx]] = exchange_idx, idx
                arr[idx], arr[exchange_idx] = arr[exchange_idx], arr[idx]
                ans += 1
        return ans
